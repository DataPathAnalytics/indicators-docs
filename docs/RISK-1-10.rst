...
Якщо в процедурі відсутні блоки ``data.questions``, для яких виконується умова, що інтервал між ``data.questions.date`` та ``data.tenderPeriod.endDate`` становить не менше 10 днів, або ``data.complaints``, що мають ``data.complaints.type='claim'``, у яких ``data.complaints.status != 'draft'`` (не дорівнює) або ``data.complaints.status != 'cancelled'`` (не дорівнює). індикатор приймає значення ``-2``, розрахунок завершується.
