...
Якщо в процедурі відсутні блоки ``data.questions`` та ``data.complaints``, що мають ``data.complaints.type='claim'``, у яких ``data.complaints.status != 'draft'`` (не дорівнює) або ``data.complaints.status != 'cancelled'`` (не дорівнює). індикатор приймає значення ``-2``, розрахунок завершується.
