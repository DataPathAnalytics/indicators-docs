############################################################################################
RISK-1-21. Порушення строку оприлюднення тендерної документації під час проведення відкритих торгів та конкурентного діалогу.
############################################################################################
...
Подія, що вмикає розрахунок індикатора - розкриття тендерних пропозицій (перехід процедури в статус ``active.qualification``).

Подія, що вимикає розрахунок індикатора
---------------------------------------

Розрахунок індикатора вимикається одразу після того, як останнє пораховане його значення дорівнює ``0`` чи ``1``.
...
Індикатор розраховується при будь-якій зміні json-документу, що відповідає процедурі, якщо виконубться умови для його розрахунку. Також, незалежно від змін у документі, індикатор розраховується раз на добу, якщо присутні всі умови для його розрахунку.
...
3. Визначена дата порівнюється з датою ``data.tenderPeriod.endDate``. Якщо дати відрізняються менше ніж на 15 днів, то індикатор приймає значення ``1``
